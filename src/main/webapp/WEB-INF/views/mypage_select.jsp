...
<body>
   ...
   <!--  전체 카테고리 -->
	...
	<div style="padding:50px 50px 50px 250px;"> 
 	<form onsubmit="return confirm('회원탈퇴 하시겠습니까?');" action="${pageContext.request.contextPath}/mypage/delete?no=${vo.memname}" method="post" id="deleteForm">
		...
		<br/><br/><br/>
			<img src="${pageContext.request.contextPath}/resources/images/face.PNG" width = "100" height = "100" /> <br/>
           	<div  style = " white-space : nowrap;  text-overflow:ellipsis;"> 
           		...
           		<input type="submit" id="deleteSuccess" value="회원탈퇴" class="btn btn-default btn-xs" /> 
           		<a href="${pageContext.request.contextPath}/mypage/qna" class="btn btn-default btn-xs"> 문의 내역</a>
           	</div> <br/> 
        </div>
    </form>
        ...
        <div class="col-sm-8 " >
			<div  style = " white-space : nowrap;  text-overflow:ellipsis;"> <h3>구매내역</h3> 		 
			 <table class="table" style="border:1px solid #cccccc">
		        <thead>
		            <tr>
		                <th style="border:1px solid #D6D6D6;">일자</th>
		                 ...
		                 <c:forEach var="vo" items="${orderlist}">
		                  <tr>
				                ...
				                <c:if test = "${vo.product.base64 ne null }">
				                 ...
				                 <td style="border:1px solid #D6D6D6;" width="100px">
				                    	${vo.count}개	
				                   </td>
				              	...
				              	<c:if test = "${vo.product.base64 eq null }">
				                    ...
				                    <td style="border:1px solid #D6D6D6;" width="150px">
			...
			$('#logout').click(function(){
				...
				$.ajax({ // 같은 기능 == axios, fetch
					...
					success: function (res) { //에러가 발생하지 않았을 경우
					    console.log(res); // ret
	                    alert("로그아웃 되었습니다."); // 알림창
	                    window.location.href='${pageContext.request.contextPath}/';
					},error: function (xhr) { //에러가 발생
					  	console.log(xhr.responseText);
						alert('로그아웃에 실패했습니다.'); // 알림창
					} 
				});
			});
		});
