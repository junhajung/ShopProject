   ...
   <!--  전체 카테고리 -->
	...
	<br/><br/><hr/>
		...
		<div class = "col-sm-9" style="height:200px, width:200px; border:1px solid #D6D6D6; background-color : #F8F9FA; "><h3> 공지사항 </h3></div>
			...
			<form  action="${pageContext.request.contextPath}/member/notice/admin?no=${vo.no}" method="post" id="orderForm" >
				<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
				
				
				<div class=" form-horizontal" style="padding:20px 20px 20px 20px; border: 1px solid #D6D6D6; height: 700px;">
				<div class="form-group">
	            
	                <h2  style="padding:0px 0px 0px 470px; "> 공지사항 수정 </h2>
	            	
	           		<div class="col-sm-12 text-left"  style="padding:0px 150px 50px 150px; ">
	              	 	  <br/> <br/>  
	              	 	  <label for="id">제목</label> 
	              	 		 <input type = "text" name = "title" placeholder = "제목" class = "form-control" value="${vo.title}"  /> <br/>
	              	 	 
	              	 	 <label for="id">내용</label> 
	              	 	 	<textarea name="content" rows="15" placeholder="공지사항을 입력하세요." class="form-control" >${vo.content}</textarea> <br/>
	              	 	  
	              	 	 <label for="id">등록일자</label>
	              	 	 	<input type = "text" name = "date" class="form-control" value = "${vo.date}" readonly/> <br/>
	              	 	 	 
	              	 	   <div class="input-group" style = "display: flex; justify-content: center; align-items: center;" >
	              	 	  <input type="submit" value="수정하기" id="orderSuccess" class="btn btn-success">
	              	 	  </div>
	            	 </div>
      			 </div>
      			 </div>
